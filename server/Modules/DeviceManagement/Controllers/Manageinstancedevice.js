...
exports.getHistoryRepairInstanceDeviceById = async (req, res) => {
  ...
  try {
    db.query(
      ...
      "select * from DeviceInstances di join tickets c on c.instance_id = di.instance_id join Devices d on d.dev_id = di.device_id join Devicetype dv on dv.devicetype_id = d.dev_type join users u on u.user_id = c.technician_id join status s on s.status_id = c.status_id where di.instance_id = ?",
