...
exports.getTechnicianRole = async (req, res) => {
  db.query(
    "SELECT u.user_img,u.user_id, u.username ,u.full_name,r.role_name,email,phone FROM users u JOIN Role r ON u.role_id = r.role_id WHERE r.role_name = 'Technician';",
    (err, result) => {
      if (err) {
        console.log(err);
